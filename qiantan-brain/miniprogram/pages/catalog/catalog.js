...
Page({
  ...
  loadSkus: function () {
    ...
    app.request({ url: '/catalog/skus' }).then(function (data) { self.setData({ skus: data || [], loading: false }); }).catch(function () { self.setData({ loading: false }); });
  },
  ...
  deleteSku: function (e) {
    ...
    wx.showModal({ title: '停用商品', content: '确认停用该SKU？', success: function (r) { if (!r.confirm) return; app.request({ url: '/catalog/skus/' + id, method: 'DELETE' }).then(function () { wx.showToast({ title: '已停用', icon: 'none' }); self.loadSkus(); }); }});
  },

  // ── 别名 ──
  loadAliases: function (e) {
    var skuId = e.currentTarget.dataset.id, self = this;
    app.request({ url: '/catalog/skus/' + skuId + '/aliases' }).then(function (data) { self.setData({ aliases: data || [], aliasSkuId: skuId, tab: 'aliases' }); });
  },
  ...
  saveAlias: function () {
    ...
    app.request({ url: '/catalog/skus/' + this.data.aliasSkuId + '/aliases', method: 'POST', data: { alias: alias } }).then(function () { self.setData({ showAliasForm: false }); wx.showToast({ title: '已添加', icon: 'success' }); self.loadAliases({ currentTarget: { dataset: { id: self.data.aliasSkuId } } }); });
  },
  removeAlias: function (e) { var id = e.currentTarget.dataset.id, self = this; app.request({ url: '/catalog/aliases/' + id, method: 'DELETE' }).then(function () { self.loadAliases({ currentTarget: { dataset: { id: self.data.aliasSkuId } } }); }); },

  // ── 规格 ──
  loadSpecs: function (e) { var skuId = e.currentTarget.dataset.id, self = this; app.request({ url: '/catalog/skus/' + skuId + '/specs' }).then(function (data) { self.setData({ specs: data || [], specSkuId: skuId, tab: 'specs' }); }); },
  ...
  saveSpec: function () {
    ...
    app.request({ url: '/catalog/skus/' + this.data.specSkuId + '/specs', method: 'POST', data: { name: f.name.trim(), price_delta: Number(f.price_delta) } }).then(function () { self.setData({ showSpecForm: false }); wx.showToast({ title: '已添加', icon: 'success' }); self.loadSpecs({ currentTarget: { dataset: { id: self.data.specSkuId } } }); });
  },
  removeSpec: function (e) { var id = e.currentTarget.dataset.id, self = this; app.request({ url: '/catalog/specs/' + id, method: 'DELETE' }).then(function () { self.loadSpecs({ currentTarget: { dataset: { id: self.data.specSkuId } } }); }); },

  // ── 单位 ──
  loadUnits: function () { var self = this; app.request({ url: '/catalog/units' }).then(function (data) { self.setData({ units: data || [] }); }); },
  loadConversions: function () { var self = this; app.request({ url: '/catalog/unit-conversions' }).then(function (data) { self.setData({ conversions: data || [] }); }); },
  ...
  saveUnit: function () {
    ...
    app.request({ url: '/catalog/units', method: 'POST', data: f }).then(function () { self.setData({ showUnitForm: false }); wx.showToast({ title: '已添加', icon: 'success' }); self.loadUnits(); });
  },
  ...
  saveConv: function () {
    ...
    app.request({ url: '/catalog/unit-conversions', method: 'POST', data: payload }).then(function () { self.setData({ showConvForm: false }); wx.showToast({ title: '已添加', icon: 'success' }); self.loadConversions(); });
  },
  deleteConv: function (e) { var id = e.currentTarget.dataset.id, self = this; app.request({ url: '/catalog/unit-conversions/' + id, method: 'DELETE' }).then(function () { self.loadConversions(); }); },
  ...
  deleteSup: function (e) { var id = e.currentTarget.dataset.id, self = this; wx.showModal({ title: '停用供应商', content: '确认停用？', success: function (r) { if (!r.confirm) return; app.request({ url: '/catalog/suppliers/' + id, method: 'DELETE' }).then(function () { wx.showToast({ title: '已停用', icon: 'none' }); self.loadSuppliers(); }); }}); },
