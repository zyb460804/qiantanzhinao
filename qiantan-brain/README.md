...
| **Web 管理后台** | 平台管理员 | React 19 + Vite + Ant Design 5 | `http://localhost:5174` |
...
| 前端（管理后台） | React 19 · Vite · Ant Design 5 · Recharts · Axios |
...
| 👤 **我的** | 经营快照、设备同步、常用 6 工具直达 + 更多工具折叠、摊位设置、帮助 |
...
| **经营报表** | 日报/周报/月报（支持历史日期回看）+ AI 点评 + 销售排行 + 趋势图 |
| **商品目录** | SKU 管理/别名/规格/单位换算/供应商比价+推荐 |
| **供应商档案** | 供应商 CRUD + 综合评分 + 黑名单 + 应付账款 |
| **财务管理** | 费用记账/月度报表/发票管理/微信&支付宝对账/数据导出 |
...
| **智能分析** | 销量异常检测、临期/库存动态定价建议、报童模型最优进货量 |
...
| 页面 | 功能 |
|------|------|
| **数据看板** | 平台概况统计（7/14/30 天）、用量趋势、套餐分布、待办提醒、最近动态 |
| **运维监控** | 数据库/设备/AI/订阅健康检查 + 趋势图 + 健康评分 + 设备列表与死信队列 |
| **租户管理** | 租户列表/详情/编辑/暂停（「新建」跳转接入向导） |
| **接入向导** | 一步创建租户+订阅+商户 |
...
│   │   ├── routers/             # 36 个路由模块（含 admin 子目录 11 个，main.py 注册 37 个路由）
...
| 报表 | `GET /reports/daily` | 日报（weekly/monthly 支持历史日期参数） |
| 员工 | `GET /staff` | 员工列表 |
...
| 功能 | 状态 | 说明 |
|------|------|------|
| 微信小程序 | ✅ | 24 个页面，覆盖经营全流程 + 智能分析 |
| FastAPI 后端 | ✅ | 36 个路由模块，248 端点，872 个测试通过 |
| Web 管理后台 | ✅ | React + Ant Design，13 个页面 |
| SaaS 多租户 | ✅ | 租户/套餐/订阅/发票/用量/门禁链 |
| 员工权限系统 | ✅ | 6 种角色 × 14 项权限 |
| 方言语音 ASR | 🟡 | 讯飞接口已接；无凭证时走 Mock |
| 语音领域解析 | ✅ | 商品/数量/金额/买卖类型；汉字数字与口语金额、多意图多笔记账 |
...
| 风险雷达 | ✅ | 六维真实计算 |
| 算法能力闭环 | ✅ | 异常检测/动态定价/报童模型已接入小程序「智能分析」，供应商页支持比价与智能推荐 |
...
| 文档 | 用途 |
|---|---|
| [文档中心索引](docs/README.md) | 全部文档按角色推荐路径导航 |
