...
async def record_waste(
    ...
    record = InventoryRecord(
        merchant_id=merchant.id, product_id=product_id, sku_id=sku_id,
        quantity=-quantity, unit=body.get("unit", product.unit),
        event_type="waste", event_time=utc_now(), source="manual",
        notes=f"{reason}: {notes}" if notes else reason,
        idempotency_key=body.get("idempotency_key") or f"waste:{merchant.id}:{product_id}:{utc_now().timestamp()}",
    )
    db.add(record)

    db.add(AuditLog(merchant_id=merchant.id, action="record_waste",
        target_table="inventory_records", target_id=str(record.id),
        after_data={"product_id": product_id, "quantity": float(quantity), "reason": reason, "notes": notes},
        reason=reason, operator="merchant"))

    await db.commit()
    return {"code": 0, "message": f"已记录{reason} {float(quantity)}{product.unit}",
...
async def list_waste(
    ...
    filters = [InventoryRecord.merchant_id == merchant.id,
               InventoryRecord.event_type == "waste"]
    if reason: filters.append(InventoryRecord.notes.like(f"{reason}%"))
    offset = (page - 1) * limit
    rows = (await db.execute(
        select(InventoryRecord).where(*filters)
        .order_by(InventoryRecord.event_time.desc()).offset(offset).limit(min(limit, 100))
    )).scalars().all()
    return {"code": 0, "data": [
        {"record_id": str(r.id), "product_id": r.product_id, "quantity": float(r.quantity),
         "unit": r.unit, "unit_cost": float(r.unit_cost) if r.unit_cost else None,
         "total_amount": float(r.total_amount) if r.total_amount else None,
         "notes": r.notes, "event_time": r.event_time.isoformat() if r.event_time else None}
        for r in rows
    ]}
...
async def waste_analysis(
    ...
    reason_rows = (await db.execute(
        select(InventoryRecord.notes, func.sum(func.abs(InventoryRecord.quantity)), func.sum(func.abs(InventoryRecord.total_amount)))
        .where(InventoryRecord.merchant_id == merchant.id, InventoryRecord.event_type == "waste",
               InventoryRecord.event_time >= since)
        .group_by(InventoryRecord.notes).order_by(func.sum(func.abs(InventoryRecord.total_amount)).desc())
    )).all()

    # By product
    product_rows = (await db.execute(
        select(InventoryRecord.product_id, func.sum(func.abs(InventoryRecord.quantity)), func.sum(func.abs(InventoryRecord.total_amount)))
        .where(InventoryRecord.merchant_id == merchant.id, InventoryRecord.event_type == "waste",
               InventoryRecord.event_time >= since)
        .group_by(InventoryRecord.product_id).order_by(func.sum(func.abs(InventoryRecord.total_amount)).desc()).limit(10)
    )).all()

    product_ids = {p for p, _, _ in product_rows}
    product_names = {}
    if product_ids:
        cats = (await db.execute(select(ProductCategory).where(ProductCategory.id.in_(product_ids)))).scalars().all()
        product_names = {c.id: c.name for c in cats}

    return {"code": 0, "data": {
        "period_days": days,
        "by_reason": [{"reason": r, "qty": float(q), "cost": float(c or 0)} for r, q, c in reason_rows],
        "by_product": [{"product_id": pid, "product_name": product_names.get(pid, f"商品{pid}"),
                        "qty": float(q), "cost": float(c or 0)} for pid, q, c in product_rows],
    }}
...
async def expiry_clearance(
    ...
    items = []
    for b in batches:
        hours_left = int((b.expiry_date - now).total_seconds() / 3600)
        risk = "high" if hours_left <= 8 else ("medium" if hours_left <= 24 else "low")
        current_price = sku_prices.get(b.sku_id)
        suggested_price = None
        if current_price:
            if risk == "high": suggested_price = float(current_price * Decimal("0.6"))
            elif risk == "medium": suggested_price = float(current_price * Decimal("0.8"))
        items.append({
            "batch_id": str(b.id), "product_name": product_names.get(b.product_id, f"商品{b.product_id}"),
            "remaining_qty": float(b.remaining_qty), "hours_left": hours_left,
            "risk": risk, "current_price": float(current_price) if current_price else None,
...
async def list_customers(
    ...
    # Get last transaction date per customer (batch query)
    result = []
    for b in balances:
        customer_name = b["customer_name"]
        profile = profiles.get(customer_name)
        last_txn = (await db.execute(
            select(CustomerReceivable.created_at).where(
                CustomerReceivable.merchant_id == merchant.id,
                CustomerReceivable.customer_name == customer_name,
            ).order_by(CustomerReceivable.created_at.desc()).limit(1)
        )).scalar_one_or_none()

        overdue_days = 0
        default_days = profile.default_credit_days if profile else 30
        if b["balance"] > 0 and last_txn:
            days_since = (utc_now() - last_txn).days
        ...
        result.append({
            ...
            "remaining_credit": round(credit_limit - b["balance"], 2) if credit_limit is not None else None,
...
async def customer_ledger(
    customer_name: str, limit: int = 50,
    merchant: Merchant = Depends(get_current_merchant),
    db: AsyncSession = Depends(get_db),
):
    ...
    rows = (await db.execute(
        select(CustomerReceivable).where(
            CustomerReceivable.merchant_id == merchant.id,
            CustomerReceivable.customer_name == customer_name,
        ).order_by(CustomerReceivable.created_at.desc()).limit(min(limit, 200))
    )).scalars().all()
    ...
    items = []
    for r in rows:
        if r.direction == "charge": total_charge += r.amount
        else: total_repay += r.amount
        items.append({
            "id": str(r.id), "direction": r.direction, "amount": float(r.amount),
            "note": r.note, "settled": r.settled,
            "created_at": r.created_at.isoformat() if r.created_at else None,
        })

    return {"code": 0, "data": {
        "customer_name": customer_name,
        "total_charge": float(total_charge), "total_repay": float(total_repay),
...
async def customer_repay(
    ...
    if not customer_name: raise HTTPException(status_code=400, detail="客户名称不能为空")
    if amount <= 0: raise HTTPException(status_code=400, detail="回款金额必须大于0")

    await record_customer_receivable(
        db, merchant_id=merchant.id, customer_name=customer_name,
        amount=amount, direction="repay", note=body.get("note", "手动回款"),
        idempotency_key=body.get("idempotency_key"),
    )
    await db.commit()
    new_balance = await get_customer_balance(db, merchant.id, customer_name)
    return {"code": 0, "message": f"{customer_name} 已回款 ¥{float(amount)}",
...
async def upsert_customer_credit_profile(
    ...
    db.add(AuditLog(
        merchant_id=merchant.id, action=f"credit_profile_{action}",
        target_table="customer_credit_profiles", target_id=str(profile.id),
        after_data={
            "customer_name": customer_name,
            "credit_limit": float(profile.credit_limit) if profile.credit_limit else None,
            "is_blocked": profile.is_blocked,
    ...
    return {"code": 0, "message": f"客户 {customer_name} 信用档案已{action == 'created' and '创建' or '更新'}",
...
async def export_sales(
    ...
    for o in orders:
        w.writerow([o.order_no, o.status, float(o.total_amount), float(o.paid_amount or 0),
                     float(o.refunded_amount or 0), o.customer_name or "",
...
async def export_inventory(
    ...
    for r in rows:
        w.writerow([product_names.get(r.product_id, f"商品{r.product_id}"),
                     float(r.current_qty), float(r.avg_cost) if r.avg_cost else ""])
...
async def export_waste(
    ...
    for r in rows:
        w.writerow([product_names.get(r.product_id, f"商品{r.product_id}"),
                     float(abs(r.quantity)), r.unit,
                     float(r.total_amount) if r.total_amount else "",
                     r.notes, r.event_time.isoformat() if r.event_time else ""])
