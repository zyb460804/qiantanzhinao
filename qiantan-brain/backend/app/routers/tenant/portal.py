...
响应格式统一为 {code: 0, data: ...}，与小程序 app.request 解包逻辑一致。
"""
...
async def get_tenant_merchant(
    ...
    过渡期（STRICT_TENANT_REQUIRED=False）：tenant_id 为空时仅记录日志，不阻断。
    """
...
@router.get("/subscription", response_model=AnyResponse)
...
@router.get("/usage/quotas", response_model=AnyResponse)
...
@router.get("/usage/trend/{metric}", response_model=AnyResponse)
