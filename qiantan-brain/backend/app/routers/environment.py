    ...
    "小暑": [
        {
            ...
            "icon": "cart",
            "tone": "info",
            "text": "西瓜、番茄进入当令旺销，西瓜备货量上调 30%。",
        },
        {
            "label": "营销",
            "icon": "spark",
            "tone": "hot",
            "text": '"消暑套餐"组合卖：西瓜+黄瓜+毛豆，客单更高。',
        },
        {
            "label": "时段",
            "icon": "leaf",
            "tone": "warn",
            "text": "午后高温，叶菜易蔫，早市多摆、晚市转清货。",
        },
    ],
    "大暑": [
        {
            ...
            "icon": "cart",
            "tone": "info",
            "text": "西瓜、番茄持续旺销，冰镇水果日备货翻倍。",
        },
        {
            "label": "营销",
            "icon": "spark",
            "tone": "hot",
            "text": '"大暑消暑日"，推凉拌菜组合，吸引街坊。',
        },
        {
            "label": "时段",
            "icon": "leaf",
            "tone": "warn",
            "text": "午间高温，叶菜类易枯，早7点前上架，11点后减量。",
        },
    ],
}
...
DEFAULT_ADVICE = [
    {
        ...
        "icon": "cart",
        "tone": "info",
        "text": "根据近期销量和天气，合理安排每日进货量。",
    },
    {
        "label": "营销",
        "icon": "spark",
        "tone": "hot",
        "text": "周末客流大时多备水果和叶菜，工作日以根茎类为主。",
    },
    {
        "label": "时段",
        "icon": "leaf",
        "tone": "warn",
        "text": "早市多摆叶菜，午市以根茎类为主，晚市打折清货。",
    },
]


def resolve_solar_term(mmdd: str) -> dict:
    """Resolve current and next solar terms for an MMDD string.

    Dates before the first January term belong to the previous year's winter
    solstice interval, so the lookup intentionally wraps across year-end.
    """
    sorted_terms = sorted(SOLAR_TERMS, key=lambda x: x[0])
    current_idx = len(sorted_terms) - 1
    for i, (term_date, _name) in enumerate(sorted_terms):
        if term_date <= mmdd:
            current_idx = i
        else:
            break

    next_idx = (current_idx + 1) % len(sorted_terms)
    term_start, current_term = sorted_terms[current_idx]
    term_end, next_term = sorted_terms[next_idx]
    return {
        "solar_term": current_term,
        "term_range": (
            term_start[:2] + "/" + term_start[2:] + " – " + term_end[:2] + "/" + term_end[2:]
        ),
        "next_term": next_term,
    }
...
async def get_solar_term():
    ...
    term_data["in_season_product_list"] = [
        name.strip() for name in term_data["in_season_products"].split("·") if name.strip()
    ]
    term_data["advice"] = SEASONAL_ADVICE.get(current_term, DEFAULT_ADVICE)
    return {"code": 0, "data": term_data}


@router.get("/seasonal", response_model=AnyResponse)
