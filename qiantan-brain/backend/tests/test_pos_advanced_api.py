...
async def _seed_stock(db_session, merchant_id=TEST_MERCHANT_ID, quantity=10, product_id=1,
                      product_name="白菜", batch_label=None):
...
class TestHoldResumeCancel:
    async def test_hold_order(self, client, db_session):
        """挂起一个订单."""
        await _seed_stock(db_session)
        res = await client.post("/api/v1/pos/orders/hold", json={
    ...
    async def test_list_held_orders(self, client, db_session):
        ...
        hold = await client.post("/api/v1/pos/orders/hold", json={
    ...
    async def test_cancel_paid_order_rejected(self, client, db_session):
        """已支付订单不能取消."""
        await _seed_stock(db_session)
        create = await client.post("/api/v1/pos/orders",
                                   json=_order("cancel-paid-001"))
    ...
    async def test_hold_does_not_consume_stock(self, client, db_session):
        ...
        await client.post("/api/v1/pos/orders/hold", json={
            "items": [{"product_id": 1, "quantity": 3, "unit": "斤", "unit_price": 4.0}],
            "client_id": "hold-stock-001",
        })
        async with db_session() as session:
            batch = (await session.execute(
                select(BatchLifecycle).where(BatchLifecycle.merchant_id == uuid.UUID(TEST_MERCHANT_ID))
            )).scalar_one()
...
class TestFullRefund:
    async def test_full_refund_with_stock_return(self, client, db_session):
        ...
        await _seed_stock(db_session, quantity=10)
        create = await client.post("/api/v1/pos/orders",
                                   json=_order("refund-full-001"))
        order_id = create.json()["data"]["order_id"]
        refund = await client.post(f"/api/v1/pos/orders/{order_id}/refund", json={
            "reason": "品质问题", "return_to_stock": True,
        })
    ...
    async def test_full_refund_without_stock_return(self, client, db_session):
        ...
        await _seed_stock(db_session, quantity=10)
        create = await client.post("/api/v1/pos/orders",
                                   json=_order("refund-no-stock-001"))
        order_id = create.json()["data"]["order_id"]
        refund = await client.post(f"/api/v1/pos/orders/{order_id}/refund", json={
            "reason": "客户不满意", "return_to_stock": False,
        })
        assert refund.status_code == 200
        async with db_session() as session:
            batch = (await session.execute(
    ...
    async def test_double_refund_rejected(self, client, db_session):
        ...
        await _seed_stock(db_session, quantity=10)
        create = await client.post("/api/v1/pos/orders",
                                   json=_order("refund-double-001"))
        order_id = create.json()["data"]["order_id"]
        await client.post(f"/api/v1/pos/orders/{order_id}/refund", json={
            "reason": "第一次退款", "return_to_stock": False,
        })
        second = await client.post(f"/api/v1/pos/orders/{order_id}/refund", json={
            "reason": "第二次退款", "return_to_stock": False,
        })
    ...
    async def test_refund_credit_order(self, client, db_session):
        """赊账订单退款应冲减应收."""
        await _seed_stock(db_session, quantity=10)
        create = await client.post("/api/v1/pos/orders", json={
        ...
        order_id = create.json()["data"]["order_id"]
        refund = await client.post(f"/api/v1/pos/orders/{order_id}/refund", json={
            "reason": "赊账客户退货", "return_to_stock": True,
        })
        assert refund.status_code == 200
...
class TestPartialRefund:
    async def test_partial_refund_single_item(self, client, db_session):
        ...
        await _seed_stock(db_session, quantity=10)
        create = await client.post("/api/v1/pos/orders",
                                   json=_order("partial-refund-001", items=[
    ...
    async def test_partial_refund_excess_quantity_rejected(self, client, db_session):
        ...
        await _seed_stock(db_session, quantity=10)
        create = await client.post("/api/v1/pos/orders",
                                   json=_order("partial-excess-001", items=[
...
class TestBatchIsolation:
    ...
    async def test_locked_batch_not_sold(self, client, db_session):
        ...
        async with db_session() as session:
            ...
            for b in batches:
                await lock_batch(session, b.id, uuid.UUID(TEST_MERCHANT_ID),
                                 "快检不合格", "merchant")
            await session.commit()
    ...
    async def test_unlocked_batch_sellable_after_recheck(self, client, db_session):
        ...
        async with db_session() as session:
            ...
            await session.commit()
        # 此时锁定 — 不可售
        res_locked = await client.post("/api/v1/pos/orders", json={
        ...
        async with db_session() as session:
            ...
            await session.commit()
        # 解锁后可售
        await _seed_stock(db_session, quantity=5, product_id=1, product_name="白菜",
                          batch_label="fresh-unlocked-002")
        res_unlocked = await client.post("/api/v1/pos/orders", json={
...
class TestUnauthenticated:
    async def test_hold_no_auth(self, auth_client):
        res = await auth_client.post("/api/v1/pos/orders/hold", json={
            "items": [{"product_id": 1, "quantity": 1, "unit": "斤", "unit_price": 3.0}],
        })
        assert res.status_code == 401

    async def test_refund_no_auth(self, auth_client):
        res = await auth_client.post(f"/api/v1/pos/orders/{uuid.uuid4()}/refund", json={
            "reason": "test",
        })
        assert res.status_code == 401
